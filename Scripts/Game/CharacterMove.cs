...
public class CharacterMove : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D final) //SKOR'DA BULUNAN COLLİSİONA ÇARPTIGINDA OYUN BİTMESİ
    {
        if (final.gameObject.CompareTag("final"))  //çarpıtıgı objenin tagi "final" ise ;
        {
    ...
    {
        if (!TasimaEngel)  //COLLİDERA ÇARPTIGINDA MAP TAŞIMA (BULUTLAR)
        {
    ...
    void Update()
    {
        yatayHareket = SimpleInput.GetAxis("Horizontal"); //UNİTY OTOMATİK TUŞ ATAMALARI a,d-sağ,sol
        ...
        if (transform.position.x >= 2.73)   //SOLDAN SAĞA GEÇERKEN SOLA ATAMASI İÇİN
        {
        ...
        if (transform.position.x <= -2.77f) //SAĞDAN SOLA GEÇERKEN SAĞA ATAMASI İÇİN
        {
            GetComponent<TrailRenderer>().enabled = false;
